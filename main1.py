...
def decrypt_from_gcs(bucket_name, source_blob_name):
    try:
        ...
        private_key = decrypt_key(os.getenv("PROJECT_ID"), os.getenv("KMS_LOCATION"), os.getenv("KEY_RING"), os.getenv("CRYPTO_KEY"), encrypted_private_key)
        ...
        pp2 = decrypt_key(os.getenv("PROJECT_ID"), os.getenv("KMS_LOCATION"), os.getenv("KEY_RING"), os.getenv("CRYPTO_KEY"), pp1)
        print("PP :{}".format(pp2))
        pp = pp2.strip()
        pp_length = len(pp)
        print("Passphrase Length: {}".format(pp_length))
        always_trust = True  # Set to True to skip key validation
        extra_args = ["--yes"]  # Additional arguments for the GPG executable
        decrypted_data = gpg.decrypt(encrypted_data, passphrase = pp, always_trust=always_trust, extra_args=extra_args)
        # decrypted_data = gpg.decrypt(encrypted_data, passphrase = pp, options=options)
        # decrypted_data = gpg.decrypt(encrypted_data, passphrase=pp)
        print("Decrypted Data")
        print(decrypted_data)
        # Check if the decryption was successful
        print("STATUS: {}".format(decrypted_data))
...
def index():
    try:
        ...
        upload_decrypted_to_gcs(os.getenv("DESTINTAION_BUCKET"), source_blob_name )
        return 'OK', 200
